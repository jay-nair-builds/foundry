...
![Verdict cards](examples/screens/02-verdict.png)
...
| | |
|---|---|
| ![Valuation brief](examples/screens/03-valuation-brief.png) | ![BBS score](examples/screens/05-bbs-score.png) |
| ![Macro](examples/screens/04-macro.png) | ![Sensitivity](examples/screens/06-sensitivity.png) |
...
- [`examples/nvda-vs-meta-scorecard.html`](examples/nvda-vs-meta-scorecard.html): the scorecard as a static page. Open it in a browser; the download button works only in Claude's published viewer.
- [`model/BBS_Valuation_Model_NVDA_META.xlsx`](model/BBS_Valuation_Model_NVDA_META.xlsx): the Excel model. Change a blue input and the value per share, verdict and BBS Score recalculate.
- [`model/engine.py`](model/engine.py) and [`model/fin.py`](model/fin.py): the Python valuation engine and the historical and forward P&L data behind the workbook. Run `python fin.py` to check that the reported operating and net income reconcile in every year.
